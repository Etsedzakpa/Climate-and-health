...
load_and_process_data <- function(health_data_path,
  ...
  # Symbol for dynamic case column
  case_sym <- rlang::sym(case_type)
  data <- data %>%
    dplyr::rename(
      year     = dplyr::all_of(year_col),
      month    = dplyr::all_of(month_col),
      region   = dplyr::all_of(region_col),
      district = dplyr::all_of(district_col),
      !!case_sym := dplyr::all_of(case_col),
      tot_pop  = dplyr::all_of(tot_pop_col)
    )
  if (!is.null(intervention_col)) {
    data <- data %>%
      dplyr::rename(intervention = dplyr::all_of(intervention_col))
  }
  if (!is.null(irs_col)) {
    data <- data %>%
      dplyr::rename(irs = dplyr::all_of(irs_col))
  }
  select_cols <- c("region", "district", "year", "month", case_type, "tot_pop")
  if (!is.null(intervention_col)) {
    select_cols <- c(select_cols, "intervention")
  }
  if (!is.null(irs_col)) {
    select_cols <- c(select_cols, "irs")
  }
  data <- data %>%
...
combine_health_climate_data <- function(health_data_path,
  ...
  case_type <- validate_case_type(case_type)
  # ---- Load health data ----
  health_data <- load_and_process_data(health_data_path = health_data_path,
                                       region_col = region_col, district_col = district_col,
                                       date_col = date_col, year_col = year_col,
                                       month_col = month_col, case_col = case_col,
                                       case_type = case_type, tot_pop_col = tot_pop_col,
                                       intervention_col = intervention_col, irs_col = irs_col
  )
  # ---- Load climate data ----
  climate_data <- load_and_process_climatedata(climate_data_path, district_col,
                                               ...
                                               ndvi_col, spi_col, max_lag
  )
  # ---- Load map ----
  map_data <- load_and_process_map(map_path, region_col = map_region_col,
                                   district_col = map_district_col,
                                   geometry_col, output_dir
  )
  # ---- Join health + climate ----
  joinby_vars <- c("district", "year", "month")
  data <- health_data %>%
    dplyr::left_join(climate_data,
                     by = dplyr::join_by(!!!rlang::syms(joinby_vars))) %>%
    dplyr::distinct() %>%
    dplyr::group_by(.data$region, .data$district) %>%
    dplyr::mutate(
      time = (.data$year - min(.data$year)) * 12 + .data$month
    ) %>%
    dplyr::ungroup()
  # ---- Create spatial grid codes ----
  grid_data <- data %>%
    dplyr::select(dplyr::all_of(c("region", "district"))) %>%
    dplyr::distinct() %>%
    dplyr::arrange(.data$region, .data$district) %>%        # consistent ordering
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      district_code = as.integer(paste0(                    # collision-free code
        region_code,
        formatC(district_number, width = 5, flag = "0")    # supports up to 99,999 districts per region
      ))
    )
  data <- dplyr::left_join(data, grid_data,
                           by = c("region", "district")) %>%
    dplyr::arrange(.data$region_code, .data$district_code)
  map <- dplyr::left_join(map_data$map,
                          grid_data,
                          by = c("region", "district")) %>%
    dplyr::arrange(.data$region_code, .data$district_code)
  ...
  # ---- Return ----
  return_list <- list(map = map, nb.map = map_data$nb.map, graph_file = map_data$graph_file,
                      data = data, grid_data = grid_data, summary = summary_stats)
...
plot_health_climate_timeseries <- function(data,
  ...
  p <- ggplot2::ggplot(agg, ggplot2::aes(date, value)) +
    ...
    ggplot2::scale_x_date(date_breaks = "12 month", date_labels = "%Y") +
...
#'
#' @keywords internal
set_cross_basis <- function(data, max_lag, nk) {
...
#'
#' @description
#' This function checks multicollinearity across the exposure term(s) of
#' interest and the additional INLA covariates using a correlation-matrix-based
#' variance inflation factor calculation. The disease outcome is intentionally
#' excluded, since VIF is only defined/meaningful among predictor variables.
#'
#' @param data A data frame containing `param_term` and the variables listed
#' in `inla_param`.
...
check_diseases_vif <- function(data,
  ...
  vars <- unique(c(param_term, inla_param))
...
check_and_write_vif <- function(data,
  ...
  VIF <- check_diseases_vif(data, param_term, inla_param)
...
plot_yearly_spatial_random_effect <- function(combined_data ,
  ...
  space <- left_join(map, space, by = c("district_code" = "district_code"), relationship = "many-to-many")
  
  if (length(filter_year) == 1 && filter_year == "All Years") {
    # All Years plot
    space_effects <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = space, 
                       ggplot2::aes(fill = mean), color = "black", size = 0.1) +
  ...
  } else if (length(filter_year) == 1 && filter_year == "Average") {
    # Average plot
    dta <- space %>%
      dplyr::group_by(district, geometry, district_number, district_code) %>% 
      dplyr::summarise(mean = mean(mean, na.rm = TRUE), .groups = "drop")
    
    space_effects <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = dta, ggplot2::aes(fill = mean), color = "black", size = 0.1) +
      ggplot2::scale_fill_gradient2(
        low = "green", mid = "white", high = "red",
        midpoint = 0,
        limits = c(min(dta$mean, na.rm = TRUE), max(dta$mean, na.rm = TRUE)),
        name = "Contribution\nto log(DIR)"
      ) +
      ggplot2::theme_void()
    
  } else {
    # Compare Years - numeric vector
    space_effects <- ggplot2::ggplot() +
      ggplot2::geom_sf(data = space %>% dplyr::filter(year %in% filter_year), 
                       ggplot2::aes(fill = mean), color = "black", size = 0.1) +
      ggplot2::scale_fill_gradient2(
        low = "green", mid = "white", high = "red",
        midpoint = 0,
  ...
  # saveRDS(space, "R/Functions/temp/space_data.rds")
...
get_predictions <- function(data,
  ...
  # Find positions of terms associated with crossbasis
  indt <- grep(paste("basis", param_term, sep = "_"), model$names.fixed)

  if (level == "country"){
    # Extract predictions centered on overall mean
    predt <- dlnm::crosspred( basis_matrices[[param_term]], coef = coef[indt],
                              vcov = vcov[indt, indt], model.link = "log",bylag = 0.25,
                              cen = round(mean(data[[param_term]], na.rm = TRUE), 0)
                              )
...
contour_plot <- function(data,
  ...
  predt <- get_predictions(data,param_term = param_term,max_lag = max_lag,nk = nk,
                           model = model,level = level,case_type = case_type
                           )
...
plot_rr_map <- function(combined_data,
  ...
  rr_all  <- dplyr::bind_rows(rr_list)
  ...
  # Region label layer
  region_label_layer <- if (!is.null(region_labels)) {
    ggplot2::geom_text(
      data = region_labels,
  ...
  # Plotting
  if (cumulative) {
    map_rr <- dplyr::left_join(map, rr_all, by = grouping_var)
    plot <- ggplot2::ggplot(map_rr) +
      ...
      region_label_layer +
      ggplot2::scale_fill_gradient2(
        low = "skyblue", mid = "white", high = "darkorange",
      ...
      ggplot2::theme_minimal() +
      ggplot2::labs(
        title = paste("Cumulative Relative Risk of",
                      tools::toTitleCase(case_type),
                      "by", tools::toTitleCase(level)),
    ...
    plots <- lapply(seq_along(years), function(i) {
      map_rr <- dplyr::left_join(map, rr_list[[i]], by = grouping_var)
...
plot_relative_risk <- function(data,
  ...
  if (!"year" %in% names(data))
    stop("'year' column not found in data.")
  ...
  output_pdf <- if (save_fig) {
    file.path(output_dir,
              paste0("RR_", param_term, "_", level, "_all_plots.pdf"))
  } else NULL

  csv_output_path <- if (save_csv) {
    file.path(output_dir,
              paste0("RR_", param_term, "_", level, "_all_plots.csv"))
  } else NULL
  ...
  build_plot <- function(pred, title) {
    ...
    ggplot2::ggplot(
      dplyr::tibble(x = pred$predvar, y = pred$allRRfit,ymin = pred$allRRlow,
                    ymax = pred$allRRhigh
                    ),
      ggplot2::aes(x, y)
    ) +
      ...
      ggplot2::geom_ribbon(
        ggplot2::aes(ymin = ymin, ymax = ymax),fill = "blue", alpha = 0.3
        ) +
  ...
  if (level == "country") {
    if (is.null(filter_year)) {
      data_all <- data
      pred <- get_predictions(data_all, param_term = param_term, max_lag = max_lag,
                              nk = nk,model = model,level = level,case_type = case_type
                              )
      ...
      rr_plot <- ggplot2::ggplot() +
        ...
        ggplot2::geom_ribbon(
          data = dplyr::tibble(x = pred$predvar,ymin = pred$allRRlow,
                               ymax = pred$allRRhigh),
        ...
        ggplot2::scale_x_continuous(limits = x_limits,
                                    breaks = x_breaks) +
        ggplot2::labs(
          title = "Relative Risk Curve (All Years Combined)",
          y = "Relative Risk"
        ) +
        ggplot2::theme_minimal() +
        ggplot2::theme(
      ...
      hist_plot <- ggplot2::ggplot(data_all,
                                   ggplot2::aes(x = !!param_sym)) +
        ...
        ggplot2::theme_minimal() +
        ggplot2::theme(
          axis.title.y = ggplot2::element_text(color = "skyblue")
        )
    ...
    if (save_csv && !is.null(csv_output_path)) {
      flat_df <- dplyr::bind_rows(
        lapply(names(all_predictions), function(yr) {
  ...
  if (level %in% c("region", "district")) {
    group_plots <- list()
    if (is.null(filter_year)) {
      preds <- get_predictions(data,param_term = param_term,max_lag = max_lag,
                               nk = nk, model = model,level = level,case_type = case_type
                               )
      ...
      for (yr in filter_year) {
        preds <- get_predictions(dplyr::filter(data, year == yr),param_term = param_term,
                                 max_lag = max_lag,nk = nk,model = model,level = level,
                                 case_type = case_type
                                 )
        all_predictions[[as.character(yr)]] <- preds

        for (grp in names(preds)) {
          p <- build_plot(preds[[grp]],
                          paste0(grp, " (", yr, ")"))
          if (!is.null(p)) {
            group_plots[[grp]] <-
              c(group_plots[[grp]], list(p))
          }
        }
      }
    }
    ...
    if (save_fig && !is.null(output_pdf)) {
      ...
      if (length(all_plots) > 0) {
        plot_pages <- split(all_plots,
                            ceiling(seq_along(all_plots) / 16))
    ...
    if (save_csv && !is.null(csv_output_path)) {
      flat_df <- dplyr::bind_rows(
        lapply(names(all_predictions), function(yr) {
          ...
          dplyr::bind_rows(
            lapply(names(preds), function(grp) {
              df <- preds[[grp]]
              dplyr::tibble(
                year = yr,
                group = grp,
...
plot_attribution_metric <- function(attr_data,
  ...
  title_lookup <- c(
    ...
    AR_Number   = paste0("Number of ", tools::toTitleCase(case_type),
                         " cases attributable to ", param_label)
  )
  
  # Used for country and region (ggplot2)
  ...
  detect_ci <- function(data, metric) {
    cols <- grep(paste0("^", metric, "(_LCI|_UCI)$"), names(data), value = TRUE)
    if (length(cols) == 2) {
      list(has_ci = TRUE,
  ...
  # Data-driven padding — adapts to any country/value magnitude
  compute_limits <- function(data, metric, ci) {
    ...
    # estimate label width from number of characters in largest value
    max_label_chars <- max(nchar(as.character(round(data[[metric]], 2))), na.rm = TRUE)
    
    # base 15% padding + 2% per character beyond 4
    pad_fraction <- 0.15 + max(0, (max_label_chars - 4)) * 0.02
    
    pad <- pad_fraction * ymax
    
    c(NA, ymax + pad)   # NA lower limit → bars always start from 0
  }
  ...
  aggregate_country <- function(df) {
    df %>%
      dplyr::group_by(year) %>%
      dplyr::summarise(
        dplyr::across(dplyr::matches("^AR_Number(_LCI|_UCI)?$"),
                      ~ sum(.x, na.rm = TRUE)),
        dplyr::across(dplyr::matches("^AR_(Fraction|per_100k)(_LCI|_UCI)?$"),
                      ~ mean(.x, na.rm = TRUE)),
        .groups = "drop"
      )
  }
  
  aggregate_spatial <- function(df, lvl) {
    df %>%
  ...
  aggregate_panel <- function(df, lvl) {
    df %>%
      dplyr::group_by(.data[[lvl]], year) %>%
      dplyr::summarise(
        AR_Fraction = mean(AR_Fraction, na.rm = TRUE),
        dplyr::across(dplyr::matches("^AR_Number(_LCI|_UCI)?$"),
                      ~ sum(.x, na.rm = TRUE)),
        dplyr::across(dplyr::matches("^AR_(per_100k)(_LCI|_UCI)?$"),
                      ~ mean(.x, na.rm = TRUE)),
        .groups = "drop"
      )
  }
  
  make_label <- function(data, metric) {
    data %>%
      dplyr::mutate(
        label_text = dplyr::case_when(
          metric == "AR_Number" & "AR_Fraction" %in% names(.) ~
            paste0(
              scales::comma(round(.data[[metric]], 0)),
              " (", scales::number(AR_Fraction, accuracy = 0.01), "%)"
            ),
          metric == "AR_Fraction" ~
            paste0(scales::number(.data[[metric]], accuracy = 0.01), "%"),
          metric == "AR_per_100k" ~
  ...
  plots <- purrr::map(metrics, function(metric) {
    ...
    if (level == "country") {
      ...
      plot_data <- aggregate_country(attr_data) %>% make_label(metric)
      if (nrow(plot_data) == 0) stop("No data available after filtering.")
      ...
      p <- ggplot2::ggplot(
        plot_data,
        ggplot2::aes(x = year, y = .data[[metric]], group = 1)
      ) +
      ...
      p <- p +
        ...
        ggplot2::labs(
          title = paste0(title, if (ci$has_ci) " (95% CI)" else ""),
          x     = "Year",
          y     = y_label
        ) +
        ggplot2::scale_y_continuous(
    ...
    if (level == "region") {
      ...
      if (is.null(filter_year)) {
        plot_data <- aggregate_spatial(attr_data, level) %>% make_label(metric)
        plot_data[[level]] <- factor(plot_data[[level]],
                                     levels = plot_data[[level]][order(plot_data[[metric]], decreasing = TRUE)])
        
        return(
          ggplot2::ggplot(plot_data,
                          ggplot2::aes(.data[[level]], .data[[metric]])) +
            ...
            ggplot2::scale_y_continuous(
              labels = formatter_lookup[[metric]],
              limits = compute_limits(plot_data, metric, detect_ci(plot_data, metric))
            ) +
            ggplot2::theme_minimal(base_size = 10) +
            ggplot2::theme(
              plot.title = ggplot2::element_text(hjust = 0.5, face = "bold")
        ...
        ordering <- plot_data %>%
          ...
          dplyr::summarise(ord = rank_fun(.data[[metric]], na.rm = TRUE), .groups = "drop") %>%
        ...
        return(
          ggplot2::ggplot(
            plot_data,
            ggplot2::aes(.data[[level]], .data[[metric]], fill = factor(year))
          ) +
    ...
    if (level == "district") {
      ...
      if (is.null(filter_year)) {
        ...
        plot_data[[level]] <- factor(plot_data[[level]], levels = plot_data[[level]])
        ...
        p <- plotly::plot_ly(
          ...
          plotly::layout(
            ...
            xaxis  = list(title = "District", tickangle = -90),
        ...
        colors <- RColorBrewer::brewer.pal(max(3, length(years)), "Set2")[seq_along(years)]
        
        p <- plotly::plot_ly()
        for (i in seq_along(years)) {
          yr    <- years[i]
          df_yr <- plot_data[plot_data$year == yr, ]
          p <- p %>%
            plotly::add_trace(
              data             = df_yr,
              x                = ~.data[[level]],
  ...
  ## ---------------- SAVE ----------------
  if (save_fig && !is.null(output_dir)) {
    purrr::walk2(plots, metrics, function(p, m) {
      fname_base <- file.path(
        output_dir,
        paste0("attribution_", level, "_", m, "_", case_type)
      )
      if (inherits(p, "gg")) {
        ggplot2::ggsave(
        ...
        htmlwidgets::saveWidget(p, paste0(fname_base, ".html"), selfcontained = TRUE)
      }
    })
  }
...
plot_avg_monthly <- function(attr_data,
                             ...
                             save_fig = FALSE,
                             output_dir = NULL) {
#   function to capitalize the beginning of a word
  capt_start <- function(x) {
    first_letter <- str_sub(x, 1, 1)
    remaining_char <- str_sub(x, 2, -1)
    
    paste0(toupper(first_letter), remaining_char)
  }
  ...
  if (save_fig) {
    if (is.null(output_dir))
      stop("'output_dir' must be provided when save_fig = TRUE")
    if (!dir.exists(output_dir))
      dir.create(output_dir, recursive = TRUE)
  }
  attr_df <- attr_data
  clim_df <- data
  if (!is.null(filter_year)) {
    attr_df <- dplyr::filter(attr_df, year %in% filter_year)
    clim_df <- dplyr::filter(clim_df, year %in% filter_year)
  }
  # Grouping level
  group_col <- switch(level,
                      country = NULL,
                      region = "region",
                      district = "district")
  ...
  for (metric in metrics) {
    ...
    y_fmt <- switch(metric,
                    AR_Number = scales::label_comma(),
                    AR_per_100k = scales::label_number(accuracy = 0.1),
                    AR_Fraction = scales::label_number(accuracy = 0.01))
    title_lab <- switch(metric,
                        AR_Number = paste(capt_start(case_type), "Attributable Number"),
                        AR_per_100k = paste(capt_start(case_type), "Attributable Rate per 100,000 population"),
                        AR_Fraction = paste(capt_start(case_type), "Attributable Fraction"))
    
    # Climate overlay helper
    add_climate <- function(p, ydat, cdat) {
      ymax <- max(ydat$value, na.rm = TRUE)
      if (!is.finite(ymax)) ymax <- 0
      pad <- 0.05 * ymax
      rng <- range(cdat$climate, na.rm = TRUE)
      if (!all(is.finite(rng))) return(p)
      if (diff(rng) == 0)
        rng[2] <- rng[1] + 1
      
      sf <- (ymax + 2 * pad) / diff(rng)
      
      cdat$climate_scaled <-
        (cdat$climate - min(rng)) * sf + pad
      p +
        ggplot2::geom_line(
          data = cdat,
    ...
    plots <- lapply(groups, function(g) {
      df <- if (is.null(group_col)) {
        attr_df
      } else {
        dplyr::filter(attr_df, .data[[group_col]] == g)
      }
      ydat <- df |>
      ...
      p <- ggplot2::ggplot(ydat,
                           ggplot2::aes(month, value)) +
        ggplot2::geom_col(fill = "steelblue") +
        ggplot2::labs(
          x = "Month",
          y = y_lab,
          subtitle = if (!is.null(g)) g else NULL,
          title = if (is.null(g)) title_lab else NULL
        ) +
        ggplot2::theme_minimal() +
        ggplot2::theme(
          axis.title.y.left = ggplot2::element_text(size = 11, face = "bold"),
          axis.title.y.right = ggplot2::element_text(size = 11, face = "bold"),
      ...
      cdat <- clim_df |>
        ...
        dplyr::group_by(month) |>
        dplyr::summarise(
          climate = mean(.data[[param_term]], na.rm = TRUE),
          .groups = "drop"
        ) |>
        dplyr::mutate(
          month = factor(month,
