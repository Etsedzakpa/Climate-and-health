...
#'
#' @keywords internal
set_cross_basis <- function(data, max_lag, nk=2) {
...
contour_plot <- function(data,
  ...
  # ---- Define lag structure directly from max_lag ----
  nlag <- max_lag
  lag_seq <- seq(0, max_lag, by = 0.25)
  ...
  if (save_fig) {
    output_file <- file.path(
      ...
      paste0("contour_plot_", param_term, "_", level,
             if (!is.null(filter_year))
               paste0("_", paste(filter_year, collapse = "_")),
             ".pdf")
    )
...
attribution_calculation <- function(data,
  ...
  ## Global RR curve
  pred_global <- dlnm::crosspred(basis_matrices[[param_term]], coef  = coef_mean[indt],
                                 vcov = vcov_full[indt, indt], model.link = "log",
                                 bylag = 0.25, cen = cen_val_global
                                 )
  ...
  for (metric in metrics) {
    ...
    plots <- lapply(groups, function(g) {
      ...
      p <- ggplot2::ggplot(ydat, ggplot2::aes(month, value)) +
        ...
        ggplot2::theme_minimal() +
        ggplot2::theme(axis.title.y.left=ggplot2::element_text(size=11, face="bold"),
                       axis.title.y.right=ggplot2::element_text(size=11, face="bold"),
                       axis.text.y.left=ggplot2::element_text(size=12, face="bold"),
                       axis.text.y.right=ggplot2::element_text(size=12, face="bold"),
